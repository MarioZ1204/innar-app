...
app.get('/api/servicios', requireAuth, async (req, res) => {
  try {
    const [estudioRows, svcRows] = await Promise.all([
      db.query('SELECT id, nombre FROM estudio_duraciones ORDER BY nombre ASC').catch(() => []),
      db.query('SELECT id, nombre FROM servicios_recibo WHERE activo=1 ORDER BY nombre ASC')
    ]);
    const seen = new Set();
    const merged = [];
    for (const r of estudioRows) {
      const key = String(r.nombre || '').trim().toLowerCase();
      if (!key || seen.has(key)) continue;
      seen.add(key);
      merged.push({ id: r.id, nombre: r.nombre, origen: 'estudio' });
    }
    for (const r of svcRows) {
      const key = String(r.nombre || '').trim().toLowerCase();
      if (!key || seen.has(key)) continue;
      seen.add(key);
      merged.push({ id: r.id, nombre: r.nombre, origen: 'recibo' });
    }
