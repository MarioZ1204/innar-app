...
/**
 * Convierte Excel Lista_Personas a CSV listo para Anexo FIDU.
 * Uso: node scripts/import-personas-xlsx.js [entrada.xlsx] [salida.csv] [salida.xlsx] [reporte.txt]
 */

const ExcelJS = require('exceljs');
const fs = require('fs');
const { mapCsvRowToPersona } = require('../utils/anexo-fidu-personas');
...
function personaToCsvCells(p) {
  return [
    p.numero_documento, p.nombres_1, p.nombres_2, p.apellidos_1, p.apellidos_2,
    p.tipo_documento, p.fecha_nacimiento, p.ciudad_nacimiento, p.genero,
    p.direccion, p.barrio, p.ciudad_residencia, p.telefono, p.correo, p.afiliacion
  ];
}
...
function scorePersona(p) {
  let s = 0;
  ['numero_documento', 'nombres_1', 'apellidos_1', 'direccion', 'telefono', 'correo'].forEach((k) => {
    if (p[k]) s += 1;
  });
  return s;
}
...
async function main() {
  ...
  for (let r = 2; r <= ws.rowCount; r++) {
    ...
    if (!cells.some((c) => String(c).trim())) { vacias += 1; continue; }
    ...
    if (entry.score > prev.score) {
      report.push(`Fila ${prev.row}: omitida — duplicado de ${p.numero_documento} (se conserva fila ${r})`);
      byDoc.set(p.numero_documento, entry);
    } else {
  ...
  personas.forEach(({ p }) => csvLines.push(personaToCsvCells(p).map(escCsv).join(',')));
  ...
  fs.writeFileSync(REPORT, [
    `Origen: ${INPUT}`,
    `Exportados: ${personas.length}`,
    `Vacías: ${vacias}, sin doc: ${sinDoc}, duplicados: ${report.filter((l) => l.includes('duplicado')).length}`,
    `CSV: ${OUT_CSV}`,
    '',
    ...report
  ].join('\r\n'), 'utf8');
